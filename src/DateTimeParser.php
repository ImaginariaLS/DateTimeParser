...
/**
 * DateTimeParser converts a date/time string to a UNIX timestamp according to the specified pattern.
 *
 ...
 * <pre>
 * $timestamp=DateTimeParser::parse('21/10/2008','dd/MM/yyyy');
 * </pre>
 *
 ...
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: DateTimeParser.php 2928 2011-02-01 17:41:51Z alexander.makarow $
 * @package system.utils
 * @since 1.0
 */

namespace Imaginaria\DateTimeParser;
...
class DateTimeParser
{
    ...
    public static function parse($value, $pattern = 'MM/dd/yyyy', $defaults = array())
    {
        ...
        if (!isset($month)){
            $month = $defaults['month'] ?? date('n');
        }

        if (!isset($day)){
            $day = $defaults['day'] ?? date('j');
        }
        ...
        if (
            ...
            if (!isset($hour)){
                $hour = $defaults['hour'] ?? date('H');
            }

            if (!isset($minute)){
                $minute = $defaults['minute'] ?? date('i');
            }

            if (!isset($second)){
                $second = $defaults['second'] ?? date('s');
            }
        ...
        if (\Imaginaria\DateTimeParser\Timestamp::isValidDate($year, $month, $day) && \Imaginaria\DateTimeParser\Timestamp::isValidTime($hour, $minute, $second)) {
            return \Imaginaria\DateTimeParser\Timestamp::getTimestamp($hour, $minute, $second, $month, $day, $year);
        } else{
            return false;
        }
    }
